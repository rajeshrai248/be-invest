            ...
            if text_file_path.exists():
                try:
                    ...
                    if content.strip():
                        ...
                        logger.info(f"📄 Loaded: {text_file_path.name} ({len(content):,} chars) → {broker.name}")
                    else:
...
def analyze_broker_costs_with_llm(broker_name: str, texts: list[dict], api_key: str, model: str = "claude-sonnet-4-6") -> dict:
    ...
    logger.info(f"🔍 Analyzing {broker_name} ({len(combined_text):,} chars) using {model}...")
...
"""
    ...
    try:
        ...
        if not response_text or not response_text.startswith('{'):
            logger.error(f"❌ Invalid or empty response from LLM for {broker_name}. Response: '{response_text[:100]}...'")
            return {"error": "Invalid or empty response from LLM"}

        analysis = json.loads(response_text)
        logger.info(f"✅ Analysis complete for {broker_name}")
        return analysis

    except Exception as e:
        logger.error(f"❌ Analysis failed for {broker_name}: {e}", exc_info=True)
        return {"error": str(e)}


def generate_comprehensive_summary(brokers: list[Broker], pdf_texts: dict[str, list], api_key: str, model: str = "claude-sonnet-4-6") -> tuple[str, dict]:
    ...
    for broker in brokers:
        if broker.name in pdf_texts:
            ...
            logger.warning(f"⚠️ No PDF text found for {broker.name}, it will be excluded from the analysis.")
    ...
    logger.info("✅ Comprehensive summary generation skipped in favor of stable JSON output.")
...
def main():
    ...
    parser.add_argument("--model", type=str, default="claude-sonnet-4-6", help="LLM model to use (default: claude-sonnet-4-6). For OpenAI use: gpt-4o")
    ...
    if not pdf_texts:
        logger.error("❌ No PDF texts found. Run the /refresh-pdfs endpoint first.")
        return 1
    ...
    logger.info(f"\n💾 Saving JSON analyses to: {json_output_path}")
    json_output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(json_output_path, "w", encoding="utf-8") as f:
        json.dump(analyses, f, indent=2, ensure_ascii=False)
    logger.info("✅ JSON analyses saved.")

    if summary and not args.json_only:
        logger.info(f"\n💾 Saving markdown summary to: {args.output}")
