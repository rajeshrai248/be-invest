...
1. Handling fees that are often missed (like Degiro's EUR1 fee)
2. Correct market-specific pricing (Brussels vs Paris/Amsterdam)
...
ENHANCED_SYSTEM_PROMPT = (
    ...
    "1. HANDLING FEES - Often overlooked but critical (e.g., EUR1 handling fee)\n"
...
def create_enhanced_prompt(broker: str, source_url: str, text: str) -> List[Dict[str, str]]:
    ...
    # Broker-specific guidance for recurring extraction mistakes.
    broker_specific_instructions = {
        "Bolero": (
            "CRITICAL: For a EUR5k Euronext Brussels trade, Bolero charges EUR15, not EUR10. "
            "Look for the correct tier structure before extracting."
        ),
        "Degiro Belgium": (
            "CRITICAL: Degiro has a EUR1 handling fee that is often missed. "
            "Look for 'handling fee', 'verwerkingskosten', or similar terms. "
            "Total cost = commission + EUR1 handling fee."
        ),
        "Degiro": (
            "CRITICAL: Degiro has a EUR1 handling fee that is often missed. "
            "Look for 'handling fee', 'verwerkingskosten', or similar terms. "
        ...
        "Rebel": (
            ...
            "For stocks up to EUR2.5k: EUR3. Verify market-specific pricing."
        ),
        "Revolut": (
        ...
        "ING Self Invest": (
            "ING Self Invest commonly uses percentage fees with a minimum. "
            "Verify the rate, minimum fee, and whether custody fees apply."
        ),
        "Keytrade Bank": (
            "Keytrade Bank uses tiered flat fees on Euronext Brussels. "
            "Look for the EUR2.45, EUR5.95, EUR14.95, then per-additional-EUR10000 structure."
        )
    }
    ...
    example_output = [
        {
            ...
            "page": 1,
            "evidence": "Transaction fee: EUR2 + 0.026% for Euronext Brussels",
...
4. HANDLING FEE DETECTION:
   ...
   - Often EUR1-EUR3 additional to main commission
...
def validate_enhanced_extraction_result(result: List[Dict]) -> List[Dict]:
    ...
    for record in result:
        ...
        if cleaned_record["handling_fee"] is not None:
            ...
            if notes:
                notes += f"; includes EUR{handling_amount} handling fee"
            else:
                notes = f"includes EUR{handling_amount} handling fee"
...
def create_broker_specific_validation_rules(broker: str) -> Dict[str, Any]:
    """Create validation rules specific to recurring broker extraction issues."""

    rules = {
        "Bolero": {
            "expected_etf_fee_5k": 15.0,  # Not EUR10
            "expected_stock_fee_5k": 15.0,  # Not EUR10
            "fee_structure": "flat",
            "validation_notes": "Verify EUR15 fee for 5k trades, not EUR10"
        },
        "Degiro Belgium": {
            "requires_handling_fee": True,
            "handling_fee_amount": 1.0,
            "fee_structure": "composite",
            "validation_notes": "Must include EUR1 handling fee"
        },
        "Degiro": {
            "requires_handling_fee": True,
            "handling_fee_amount": 1.0,
